...
subroutine read_GcosmoBH_particles()
  ...
  files: do fn = 0, shead%nfiles-1
     ...
     deallocate(rblck)
     write(*,*) "Min/Max rho = ", minval(psys%par(:)%rho), maxval(psys%par(:)%rho)
     ...
     deallocate(rblck)
     write(*,*) "Min/Max reh = ", minval(psys%par(:)%ye), maxval(psys%par(:)%ye)
     ...
     deallocate(rblck)
     write(*,*) "Min/Max rnh = ", minval(psys%par(:)%xHI), maxval(psys%par(:)%xHI)
     ...
     deallocate(rblck)
     write(*,*) "Min/Max sml = ", minval(psys%par(:)%hsml), maxval(psys%par(:)%hsml)
